...
# Tomato and strawberry are *ongoing*: they yield on a schedule and the plant
# stays. Production does not depend on watering — only survival does — so they
# cost a watering every other day and one harvest at the end, which is cheap
# for the tile-days they occupy. They earn their place because the town eats
# strawberry (brunch spots, ice cream, smoothies, farmers markets) and melon
# has a hard ceiling, so the last tiles of a big farm are worth more in a crop
# whose price the shops hold up.
CROPS = {
    "WHEAT":      {"seed": 10,  "first": 2,  "max_day": 4,  "peak_age": 4,
                   "peak_yield": 4, "actions": 6,  "ongoing": False},
    "CARROT":     {"seed": 20,  "first": 2,  "max_day": 3,  "peak_age": 3,
                   "peak_yield": 3, "actions": 5,  "ongoing": False},
    "MELON":      {"seed": 80,  "first": 10, "max_day": 12, "peak_age": 10,
                   "peak_yield": 6, "actions": 12, "ongoing": False},
    "TOMATO":     {"seed": 50,  "first": 8,  "max_day": 8,  "peak_age": 11,
                   "peak_yield": 4, "actions": 8,  "ongoing": True},
    "STRAWBERRY": {"seed": 100, "first": 10, "max_day": 10, "peak_age": 16,
                   "peak_yield": 4, "actions": 11, "ongoing": True},
}
...
# What each town shop eats. A shop that wants only one product eats two of it
# per tick; the town centre eats one of everything but fertiliser, once a day.
SHOPS = {
    "BAKERY":         ("EGG", "WHEAT"),
...
SHOP_TICKS_PER_DAY = 6.0        # a tick every four of the twenty-four turns
TOWN_CENTRE = tuple(k for k in
...
class Econ:
    """Prices the *next* unit, against both sides of the market.

    Supply is what the farm already owes: standing crops, placed animals,
    livestock still in the shed. Pricing without it says every empty tile wants
    the same crop, so the farm monocrops and drowns its own market.

    Demand is the town, and missing it was worse. The shops in
    `town.unlocked_shops` eat a fixed basket every four turns for the rest of
    the season — a single-product shop eats two — and three yarn stores get
    through thirty-six wool a day. Against a market nobody supplies that holds
    wool near its base price all season, while an agent that models inventory
    as only ever rising values the same wool at a dollar and never keeps a
    sheep. The competition's own notebook titles say as much: shape the shop,
    work the pasture.
    """

    def __init__(self, market_inv, standing, day, money, shops=()):
        ...
        for name in shops or ():
            wants = SHOPS.get(name)
            if not wants:
                continue
            each = 2.0 if len(wants) == 1 else 1.0
            for item in wants:
                self.eaten[item] = self.eaten.get(item, 0.0) + each

    def drain(self, item, days):
        """Units the town will eat over the next `days` days.

        Conservative: it counts the shops open now, and more unlock every three
        days to a cap of eight.
        """
        if days <= 0:
            return 0.0
        per_day = self.eaten.get(item, 0.0) * SHOP_TICKS_PER_DAY
        if item in TOWN_CENTRE:
            per_day += 1.0
        return per_day * days
    ...
    def batch(self, item, n, delay=0.0, spread=0.0):
        """Coins for `n` more units, sold at the tail of what we already owe.

        `delay` is how long before the first one is ready and `spread` how long
        the run takes to arrive — the town is eating throughout both, so the
        inventory these sell into is lower than today's. When the town eats
        faster than we produce the price climbs as we sell rather than falling.
        """
        n = int(n)
        if n <= 0:
            return 0.0
        base = self._floor(item) - self.drain(item, delay)
        step = 1.0 - self.drain(item, spread) / n
        return float(sum(price_at(item, int(base + i * step)) for i in range(n)))
...
def crop_value(crop, day, econ):
    ...
    if not c or day + c["peak_age"] > LAST_DAY:
        return None
    return econ.batch(crop, c["peak_yield"], delay=c["peak_age"]) - c["seed"]
...
def animal_value(kind, day, econ):
    ...
    product = econ.batch(a["product"], units, delay=a["first"], spread=producing)
    fertiliser = econ.batch("FERTILIZER", alive, spread=alive)
...
def bonus_window(crop):
    ...
    Only one-time crops have one. An ongoing crop produces on its schedule
    whether or not it was watered that day, so watering it buys nothing but
    survival — and survival only needs every other day.
    """
    c = CROPS.get(crop)
    if not c or c["ongoing"]:
        return (99, -1)
    return ((c["max_day"] + 1) // 2, c["max_day"])
...
def build_tasks(tiles, day, econ, seeds, shed, n_units, hungry_value=0.0,
    ...
    tasks = []
    empties = []
    # Pens standing empty and livestock still in the shed are both already
    # claims on the same labour, so they come off the cap too. Counting only
    # the animals actually placed let one game lay down thirty-six pastures
    # for ten animals — thirty-six tiles and thirty-six turns for nothing.
    committed = stocked + sum((empty_pens or {}).values()) \
        + sum((shed or {}).get(k, 0) for k in ANIMALS)
    pens_left = min(pen_budget, max(0, BIRDS_PER_UNIT * n_units - committed))
